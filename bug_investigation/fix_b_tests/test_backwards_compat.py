...
def test_legacy_backup_loads_without_accumulator_file():
    ...
    t1 = ShadowTrainer(
        ...
        concepts=[{"name": "c", "path": "/x", "seed": 0, "type": "image", "include_subdirectories": False}],
    )
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert not os.path.exists(os.path.join(td, "accumulator")), (
            "shadow trainer wrote an accumulator file with enable_fix=False; test premise broken"
        )
        ...
        assert not warnings, f"unexpected warning loading legacy backup: {warnings}"
