...
def _build(enable_fix: bool, seed: int = SEED):
    ...
    return ShadowTrainer(
        ...
        concepts=[{"name": "c1", "path": "/p/a", "seed": 1, "type": "image", "include_subdirectories": False}],
    )
...
def test_only_trainable_params_in_saved_grads():
    ...
    with tempfile.TemporaryDirectory() as td:
        t.save_to(td)
        payload = torch.load(os.path.join(td, "accumulator", "accumulator.pt"), weights_only=False)
        saved_grad_keys = set(payload["param_grads"].keys())

    # Only the last Linear's weight + bias should appear.
    trainable_keys = {n for n, p in t.model.named_parameters() if p.requires_grad}
    assert saved_grad_keys == trainable_keys, f"saved keys {saved_grad_keys} != trainable keys {trainable_keys}"
    # The frozen base must NOT leak into the payload.
    frozen_keys = {n for n, p in t.model.named_parameters() if not p.requires_grad}
    assert not (saved_grad_keys & frozen_keys), f"frozen params leaked into payload: {saved_grad_keys & frozen_keys}"
...
def test_adapter_bit_identical_baseline_vs_restart():
    ...
    for step in sorted(base_d):
        assert base_d[step] == rest_d[step], f"step {step}: baseline={base_d[step]} restart={rest_d[step]}"
