    ...
    const resolvedSections = useMemo(() => {
        ...
        if (tab.variants && tab.variants.length > 0 && tab.sectionDefs) {
            ...
            if (tab.layout === "three-column") {
                return matchedVariant.columns.map((col) =>
                ...
                if (field.key === "loss_weight_fn") {
                    ...
                    if (modelType) {
                        ...
                        return values.filter(
                            (v) =>
                                v === "CONSTANT" ||
                                LOSS_WEIGHT_SUPPORTS_FLOW_MATCHING[v as LossWeight] === isFlowMatching,
                        );
                    }
                }
                return values;
    ...
    const renderSection = (section: SectionDef) => {
        ...
        return (
            <SectionCard key={section.id} title={section.label}>
                <div
                    className={
                        tab.layout === "three-column" ? "flex flex-col gap-4" : "grid grid-cols-1 md:grid-cols-2 gap-4"
                    }
                >
                    {fields.map((field) => (
                        <SchemaField
                        ...
                        <div className="flex items-center gap-2">
                            <Button
                                ...
                                onClick={async () => {
                                    ...
                                    if (
                                        confirm(
                                            `Remove ${preview.total_files} orphaned cache files (${preview.total_mb} MB)?`,
                                        )
                                    ) {
        ...
        <>
            {tab.layout === "three-column" ? (
                ...
                <div className="flex flex-col gap-6">
                    {resolvedSections.flat().map((section) => renderSection(section))}
                </div>
            )}
