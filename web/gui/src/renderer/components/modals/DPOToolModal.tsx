            ...
            {result.pairs.map((pair, i) => (
                <div key={i} className="text-xs p-2 rounded bg-[var(--color-surface-container)] space-y-0.5">
                    ...
                    <div className="text-[var(--color-on-surface-secondary)]">
                        Matched: {pair.matched}, Chosen stray: {pair.chosen_stray}, Rejected stray:{" "}
                        {pair.rejected_stray}
                    </div>
                </div>
            ))}
        ...
        return (
            <div className="space-y-4">
                ...
                <p className="text-xs text-[var(--color-on-surface-secondary)]">
                    Point to a folder of generated images with prompt metadata. Images are grouped by prompt, then you
                    pick the best and worst from each group to create chosen/rejected pairs.
                </p>
                <div className="space-y-3">
                    <div>
        ...
        return (
            <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                    <div className="text-xs text-[var(--color-on-surface-secondary)]">
                        Group {group.group_index} / {group.total_groups} &middot; AR: {group.aspectratio} &middot; ELO
                        comparisons {eloDone}/{eloSuggested}
                    </div>
                    <div className="flex gap-1">
            ...
            <div className="space-y-3">
                ...
                >
                    {phase === "best" ? (
                        ...
                        <>
                            Best selected: {bestImage ? bestImage.split(/[/\\]/).pop() : ""}. Now click to select WORST
                            (rejected)
                        </>
                    )}
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 max-h-[400px] overflow-y-auto">
                    {images.map((imgPath) => (
                        <button
                            ...
                            style={{
                                borderColor: imgPath === bestImage ? "#22c55e" : "var(--color-border-subtle)",
                            }}
                ...
                {finalResult ? (
                    <>
                        ...
                        <p className="text-sm text-[var(--color-on-surface-secondary)]">
                            {finalResult.total_pairs} pairs ({finalResult.train_count} train, {finalResult.val_count}{" "}
                            val)
                        </p>
...
export function DPOToolModal({ open, onClose }: Props) {
    ...
    const handleRemovePair = useCallback(async (chosen: string, rejected: string) => {
        try {
            ...
            setReviewPairs(
                (prev) => prev?.filter((p) => p.chosen_path !== chosen || p.rejected_path !== rejected) ?? null,
            );
                ...
                {!checkResult && !reviewPairs && !loading && !error && (
                    <div className="text-center py-8 text-[var(--color-on-surface-secondary)]">
                        ...
                        <p className="text-xs mt-1">
                            Use <strong>Selection Mode</strong> to score generated images and create pairs, or use the
                            other tools to manage existing pairs.
                        </p>
                    </div>
                )}
            </div>
