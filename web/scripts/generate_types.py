...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "web", "gui", "src", "renderer", "types", "generated")
...
def generate_config_ts(configs: list[tuple[str, type]], enum_names: set[str]) -> str:
    lines = [
        "// Auto-generated by web/scripts/generate_types.py",
        ...
        for field_type in instance.types.values():
            if issubclass_safe(field_type, BaseConfig) and field_type.__name__ not in generated:
                ...
                if (
                    args
                    and len(args) > 1
                    and issubclass_safe(args[1], BaseConfig)
                    and args[1].__name__ not in generated
                ):
                    for cn, cc in configs:
...
def generate_metadata_ts(configs: list[tuple[str, type]], enum_names: set[str]) -> str:
    lines = [
        "// Auto-generated by web/scripts/generate_types.py",
        "// Do not edit manually. Regenerate when backend config classes change.",
        "",
...
def generate_model_type_info_ts(enums: list[tuple[str, type]]) -> str:
    ...
    for method_name in group_methods:
        members_in_group = [mt.value for mt in ModelType if _safe_call_method(mt, method_name)]
        if members_in_group:
    ...
    for mt in ModelType:
        flags = [method_name for method_name in group_methods if _safe_call_method(mt, method_name)]
...
def generate_layer_presets_ts() -> str:
    ...
    try:
        ...
        for args, _kwargs, cls in registry:
            ...
            if raw is None:
                continue
            presets_by_model[mt.value] = {str(k): _normalize_layer_preset_value(v) for k, v in raw.items()}
            seen.add(mt)
...
def generate_optimizer_info_ts(enums: list[tuple[str, type]]) -> str:
    ...
    for opt in Optimizer:
        ...
        lines.append(
            f"  {_js_key(opt.value)}: {{ isAdaptive: {a}, isScheduleFree: {sf}, supportsFusedBackPass: {fb} }},"
        )
    lines.append("};")
    lines.append("")
...
def generate_enum_labels_ts(enums: list[tuple[str, type]]) -> str:
    ...
    lines.append("      (_match, first: string, rest: string) => first.toUpperCase() + rest.toLowerCase(),")
    lines.append("    );")
    lines.append("}")
    lines.append("")
...
def _extract_options_kv_calls(filepath: str, enum_lookup: dict[str, type]) -> dict[str, list[tuple[str, str]]]:
