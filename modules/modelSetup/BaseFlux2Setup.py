...
class BaseFlux2Setup(
    ...
    def setup_optimizations(
        ...
        if config.gradient_checkpointing.enabled():
            model.transformer_offload_conductor = enable_checkpointing_for_flux2_transformer(model.transformer, config)
            if model.text_encoder is not None:
                if model.is_dev():
                    model.text_encoder_offload_conductor = enable_checkpointing_for_mistral_encoder_layers(
        ...
        with model.autocast_context:
            ...
            if model.transformer.config.guidance_embeds:
                guidance = torch.tensor(
                    [config.transformer.guidance_scale], device=self.train_device, dtype=model.train_dtype.torch_dtype()
                )
