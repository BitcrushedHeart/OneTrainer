...
class QwenBaseDataLoader(
    ...
    def _preparation_modules(self, config: TrainConfig, model: QwenModel):
        ...
        encode_prompt = EncodeQwenText(
            ...
            trim_padding=text_caching_active,
            batch_collector=text_caching_active and text_encode_batch > 1,
    ...
    def _cache_modules(self, config: TrainConfig, model: QwenModel, model_setup: BaseQwenSetup):
        ...
        return self._cache_modules_from_names(
            ...
            text_cache_build_workers=text_encode_batch_size()
            if config.latent_caching and not config.train_text_encoder_or_embedding()
            else None,
        )

    def _output_modules(
        self, config: TrainConfig, model: QwenModel, model_setup: BaseQwenSetup, is_validation: bool = False
    ):
