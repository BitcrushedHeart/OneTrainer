...
class WuerstchenBaseDataLoader(
    ...
    def _preparation_modules(self, config: TrainConfig, model: WuerstchenModel):
        ...
        normalize_image = NormalizeImageChannels(
            image_in_name="image", image_out_name="image", mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)
        )
        encode_image = EncodeWuerstchenEffnet(
            in_name="image",
            out_name="latent_image",
            effnet_encoder=model.effnet_encoder,
            autocast_contexts=[model.autocast_context, model.effnet_encoder_autocast_context],
            dtype=model.effnet_encoder_train_dtype.torch_dtype(),
        )
        ...
        add_embeddings_to_prompt = MapData(
            in_name="prompt", out_name="prompt", map_fn=model.add_prior_text_encoder_embeddings_to_prompt
        )
        tokenize_prompt = Tokenize(
            ...
            max_token_length=model.prior_tokenizer.model_max_length,
        )
        if model.model_type.is_wuerstchen_v2():
            encode_prompt = EncodeClipText(
                in_name="tokens",
                tokens_attention_mask_in_name="tokens_mask",
                hidden_state_out_name="text_encoder_hidden_state",
                pooled_out_name=None,
                add_layer_norm=True,
                text_encoder=model.prior_text_encoder,
            ...
            encode_prompt = EncodeClipText(
                ...
                tokens_attention_mask_in_name="tokens_mask",
                hidden_state_out_name="text_encoder_hidden_state",
                pooled_out_name="pooled_text_encoder_output",
