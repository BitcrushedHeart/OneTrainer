...
class ConceptWindow(ctk.CTkToplevel):
    ...
    def __general_tab(self, master, concept: ConceptConfig):
        ...
        # name
        components.label(frame, 0, 0, "Name", tooltip="Name of the concept")
        components.entry(frame, 0, 1, self.ui_state, "name")

        # enabled
        components.label(frame, 1, 0, "Enabled", tooltip="Enable or disable this concept")
        ...
        # path
        components.label(frame, 3, 0, "Path", tooltip="Path where the training data is located")
        ...
        components.button(
            ...
            2,
            text="download now",
            command=self.__download_dataset_threaded,
        ...
        components.options_kv(
            ...
            self.text_ui_state,
            "prompt_source",
            command=set_prompt_path_entry_enabled,
        )
        set_prompt_path_entry_enabled(concept.text.prompt_source)

        # include subdirectories
        components.label(
            frame, 5, 0, "Include Subdirectories", tooltip="Includes images from subdirectories into the dataset"
        )
        ...
        # loss weight
        components.label(frame, 9, 0, "Loss Weight", tooltip="The loss multiplyer for this concept.")
    ...
    def __image_augmentation_tab(self, master):
        ...
        # header
        components.label(frame, 0, 1, "Random", tooltip="Enable this augmentation with random values")
        components.label(frame, 0, 2, "Fixed", tooltip="Enable this augmentation with fixed values")
        ...
        preview_augmentations_switch = ctk.CTkSwitch(
            update_button_frame,
            text="Show Augmentations",
            variable=self.preview_augmentations,
        ...
        self.filename_preview = ctk.CTkLabel(
            master=update_button_frame,
            text=filename_preview,
            width=300,
            anchor="nw",
            justify="left",
            padx=10,
        ...
        self.caption_preview = ctk.CTkTextbox(
            master=update_button_frame, width=300, height=150, wrap="word", border_width=2
        )
    ...
    def __concept_stats_tab(self, master):
        ...
        # file size
        self.file_size_label = components.label(
            frame, 1, 0, "Total Size", pad=0, tooltip="Total size of all image, mask, and caption files in MB"
        )
        ...
        # subdirectory count
        self.dir_count_label = components.label(
            frame,
            1,
            1,
            "Directories",
        ...
        self.image_count_label = components.label(
            frame,
            3,
            0,
            "\nTotal Images",
            pad=0,
            tooltip="Total number of image files, any of the extensions "
            + str(path_util.SUPPORTED_IMAGE_EXTENSIONS)
            + ", excluding '-masklabel.png and -condlabel.png'",
        )
        ...
        self.video_count_label = components.label(
            frame,
            3,
            1,
            "\nTotal Videos",
        ...
        self.mask_count_label = components.label(
            frame,
            3,
            2,
            "\nTotal Masks",
        ...
        self.caption_count_label = components.label(
            frame,
            3,
            3,
            "\nTotal Captions",
        ...
        self.image_count_mask_label = components.label(
            frame, 5, 0, "\nImages with Masks", pad=0, tooltip="Total number of image files with an associated mask"
        )
        ...
        self.mask_count_label_unpaired = components.label(
            frame,
            5,
            1,
            "\nUnpaired Masks",
        ...
        self.image_count_caption_label = components.label(
            frame,
            7,
            0,
        ...
        self.video_count_caption_label = components.label(
            frame,
            7,
            1,
        ...
        self.caption_count_label_unpaired = components.label(
            frame,
            7,
            2,
            "\nUnpaired Captions",
        ...
        # resolution info
        self.pixel_max_label = components.label(
            frame,
            9,
            0,
            "\nMax Pixels",
        ...
        self.pixel_avg_label = components.label(
            frame,
            9,
            1,
            "\nAvg Pixels",
        ...
        self.pixel_min_label = components.label(
            frame,
            9,
            2,
            "\nMin Pixels",
        ...
        # video length info
        self.length_max_label = components.label(
            frame, 11, 0, "\nMax Length", pad=0, tooltip="Longest video in the concept by number of frames"
        )
        ...
        self.length_avg_label = components.label(
            frame, 11, 1, "\nAvg Length", pad=0, tooltip="Average length of videos in the concept by number of frames"
        )
        ...
        self.length_min_label = components.label(
            frame, 11, 2, "\nMin Length", pad=0, tooltip="Shortest video in the concept by number of frames"
        )
        ...
        # video fps info
        self.fps_max_label = components.label(
            frame, 13, 0, "\nMax FPS", pad=0, tooltip="Video in concept with highest fps"
        )
        ...
        self.fps_avg_label = components.label(
            frame, 13, 1, "\nAvg FPS", pad=0, tooltip="Average fps of videos in the concept"
        )
        ...
        self.fps_min_label = components.label(
            frame, 13, 2, "\nMin FPS", pad=0, tooltip="Video in concept with the lowest fps"
        )
        ...
        # caption info
        self.caption_max_label = components.label(
            frame,
            15,
            0,
            "\nMax Caption Length",
        ...
        self.caption_avg_label = components.label(
            frame,
            15,
            1,
            "\nAvg Caption Length",
        ...
        self.caption_min_label = components.label(
            frame,
            15,
            2,
            "\nMin Caption Length",
        ...
        # aspect bucket info
        self.aspect_bucket_label = components.label(
            frame,
            17,
            0,
            "\nAspect Bucketing",
        ...
        self.small_bucket_label = components.label(
            frame,
            17,
            1,
            "\nSmallest Buckets",
        ...
        background_color = (
            f"#{int(background_color[0] / 256):x}{int(background_color[1] / 256):x}{int(background_color[2] / 256):x}"
        )
        ...
        self.refresh_basic_stats_button = components.button(
            master=frame,
            row=0,
            column=0,
            text="Refresh Basic",
        ...
        self.refresh_advanced_stats_button = components.button(
            master=frame,
            row=0,
            column=1,
            text="Refresh Advanced",
            command=lambda: self.__get_concept_stats_threaded(True, 9999),
            tooltip="Reload advanced statistics for the concept directory",
        )  # run "basic" scan first before "advanced", seems to help the system cache the directories and run faster
        self.cancel_stats_button = components.button(
            master=frame,
            row=0,
            column=2,
            text="Abort Scan",
        ...
        self.processing_time = components.label(
            frame, 0, 3, text="-", tooltip="Time taken to process concept directory"
        )
    ...
    def __get_preview_image(self):
        ...
        if self.preview_augmentations.get():
            ...
            circular_mask_shrink = RandomCircularMaskShrink(
                ...
                shrink_factor_min=0.2,
                shrink_factor_max=1.0,
            ...
            random_flip = RandomFlip(
                names=["image", "mask"], enabled_in_name="enable_random_flip", fixed_enabled_in_name="enable_fixed_flip"
            )
            ...
            drop_tags = DropTags(
                text_in_name="prompt",
                enabled_in_name="tag_dropout_enable",
                probability_in_name="tag_dropout_probability",
                dropout_mode_in_name="tag_dropout_mode",
                ...
                keep_tags_count_in_name="keep_tags_count",
                text_out_name="prompt",
                regex_enabled_in_name="tag_dropout_special_tags_regex",
            )
            caps_randomize = CapitalizeTags(
                text_in_name="prompt",
                enabled_in_name="caps_randomize_enable",
            ...
            shuffle_tags = ShuffleTags(
                text_in_name="prompt",
                enabled_in_name="enable_tag_shuffling",
                delimiter_in_name="tag_delimiter",
                keep_tags_count_in_name="keep_tags_count",
                text_out_name="prompt",
            )
    ...
    def __update_concept_stats(self):
        ...
        if self.concept.concept_stats["subcaption_count"] > 0:
            self.caption_count_preview.configure(
                text=f"{self.concept.concept_stats['caption_count']} ({self.concept.concept_stats['subcaption_count']})"
            )
        else:
        ...
        if (
            any(isinstance(x, str) for x in [max_pixels, avg_pixels, min_pixels])
            or self.concept.concept_stats["image_count"] == 0
        ):  # will be str if adv stats were not taken
            ...
            self.pixel_max_preview.configure(
                text=f"{str(round(max_pixels[0] / 1000000, 2))} MP, {max_pixels[2]}\n{max_pixels[1]}"
            )
            self.pixel_avg_preview.configure(
                text=f"{str(round(avg_pixels / 1000000, 2))} MP, ~{int(math.sqrt(avg_pixels))}w x {int(math.sqrt(avg_pixels))}h"
            )
            self.pixel_min_preview.configure(
                text=f"{str(round(min_pixels[0] / 1000000, 2))} MP, {min_pixels[2]}\n{min_pixels[1]}"
            )
        ...
        if (
            any(isinstance(x, str) for x in [max_length, avg_length, min_length])
            or self.concept.concept_stats["video_count"] == 0
        ):  # will be str if adv stats were not taken
        ...
        if (
            any(isinstance(x, str) for x in [max_caption_length, avg_caption_length, min_caption_length])
            or self.concept.concept_stats["caption_count"] == 0
        ):  # will be str if adv stats were not taken
            ...
            self.caption_max_preview.configure(
                text=f"{max_caption_length[0]} chars, {max_caption_length[2]} words\n{max_caption_length[1]}"
            )
            self.caption_avg_preview.configure(
                text=f"{int(avg_caption_length[0])} chars, {int(avg_caption_length[1])} words"
            )
            self.caption_min_preview.configure(
                text=f"{min_caption_length[0]} chars, {min_caption_length[2]} words\n{min_caption_length[1]}"
            )

        # aspect bucketing
        aspect_buckets = self.concept.concept_stats["aspect_buckets"]
        if (
            len(aspect_buckets) != 0 and max(val for val in aspect_buckets.values()) > 0
        ):  # check aspect_bucket data exists and is not all zero
            min_val = min(val for val in aspect_buckets.values() if val > 0)  # smallest nonzero values
            if (
                max(val for val in aspect_buckets.values()) > min_val
            ):  # check if any buckets larger than min_val exist - if all images are same aspect then there won't be
                min_val2 = min(
                    val for val in aspect_buckets.values() if (val > 0 and val != min_val)
                )  # second smallest bucket
            else:
    ...
    def __get_concept_stats(self, advanced_checks: bool, wait_time: float):
        ...
        for path in subfolders:
            ...
            stats_dict = concept_stats.folder_scan(
                path, stats_dict, advanced_checks, self.concept, start_time, wait_time, self.cancel_scan_flag
            )
            if (
                self.concept.include_subdirectories and not self.cancel_scan_flag.is_set()
            ):  # add all subfolders of current directory to for loop
    ...
    def __get_concept_stats_threaded(self, advanced_checks: bool, waittime: float):
        self.scan_thread = threading.Thread(
            target=self.__get_concept_stats, args=[advanced_checks, waittime], daemon=True
        )
        self.scan_thread.start()
