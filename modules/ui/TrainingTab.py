...
class TrainingTab:
    ...
    def __create_base_frame(self, master, row):
        ...
        # optimizer
        components.label(frame, 0, 0, "Optimizer", tooltip="The type of optimizer")
        components.options_adv(
            ...
            self.ui_state,
            "optimizer.optimizer",
            command=self.__restore_optimizer_config,
            adv_command=self.__open_optimizer_params_window,
        )
        ...
        _, d = components.options_adv(
            frame,
            1,
            1,
            [str(x) for x in list(LearningRateScheduler)],
            self.ui_state,
            "learning_rate_scheduler",
            command=self.__restore_scheduler_config,
        ...
        # learning rate
        components.label(frame, 2, 0, "Learning Rate", tooltip="The base learning rate")
        ...
        components.entry(
            ...
            extra_validate=check_range(
                lower=0, upper=0.99, message="Learning rate min factor must be between 0 and 0.99"
            ),
        )
        ...
        # epochs
        components.label(frame, 6, 0, "Epochs", tooltip="The number of epochs for a full training run")
        ...
        components.options(
            frame, 9, 1, [str(x) for x in list(LearningRateScaler)], self.ui_state, "learning_rate_scaler"
        )

        # clip grad norm
        components.label(
            frame, 10, 0, "Clip Grad Norm", tooltip="Clips the gradient norm. Leave empty to disable gradient clipping."
        )
        components.entry(frame, 10, 1, self.ui_state, "clip_grad_norm")

    def __create_base2_frame(
        self, master, row, video_training_enabled: bool = False, supports_circular_padding: bool = False
    ):
        frame = ctk.CTkFrame(master=master, corner_radius=5)
        frame.grid(row=row, column=0, padx=5, pady=5, sticky="nsew")
        frame.grid_columnconfigure(0, weight=1)
        ...
        components.entry(
            ...
            self.ui_state,
            "ema_decay",
            extra_validate=check_range(lower=0.5, upper=1, message="EMA decay must be between 0.5 and 1"),
        )
        row += 1

        # ema update step interval
        components.label(frame, row, 0, "EMA Update Step Interval", tooltip="Number of steps between EMA update steps")
        ...
        components.options_adv(
            ...
            adv_command=self.__open_offloading_window,
        )
        row += 1
        ...
        components.entry(
            frame, row, 1, self.ui_state, "resolution", required=True, extra_validate=validate_resolution()
        )
        row += 1

        # frames
        if video_training_enabled:
            components.label(frame, row, 0, "Frames", tooltip="The number of frames used for training.")
    ...
    def __create_text_encoder_frame(
        self, master, row, supports_clip_skip=True, supports_training=True, supports_sequence_length=False
    ):
        frame = ctk.CTkFrame(master=master, corner_radius=5)
        frame.grid(row=row, column=0, padx=5, pady=5, sticky="nsew")
        frame.grid_columnconfigure(0, weight=1)
        ...
        if supports_training:
            components.label(frame, row, 0, "Train Text Encoder", tooltip="Enables training the text encoder model")
        ...
        if supports_training:
            ...
            components.label(frame, row, 0, "Stop Training After", tooltip="When to stop training the text encoder")
    ...
    def __create_text_encoder_n_frame(
        ...
        if supports_include:
            ...
            components.label(
                frame, row, 0, f"Include Text Encoder {i}", tooltip=f"Includes text encoder {i} in the training run"
            )
        ...
        # train text encoder
        components.label(
            frame, row, 0, f"Train Text Encoder {i}", tooltip=f"Enables training the text encoder {i} model"
        )
        ...
        components.label(
            ...
            row,
            0,
            f"Train Text Encoder {i} Embedding",
        ...
        # train text encoder epochs
        components.label(frame, row, 0, "Stop Training After", tooltip=f"When to stop training the text encoder {i}")
        components.time_entry(
            frame,
            row,
            1,
            self.ui_state,
            f"text_encoder{suffix}.stop_training_after",
            f"text_encoder{suffix}.stop_training_after_unit",
            supports_time_units=False,
        )
        row += 1
        ...
        components.label(
            ...
            row,
            0,
            f"Text Encoder {i} Learning Rate",
        ...
        if supports_sequence_length:
            ...
            components.label(
                ...
                row,
                0,
                f"Text Encoder {i} Sequence Length",
    ...
    def __create_unet_frame(self, master, row):
        ...
        # train unet
        components.label(frame, 0, 0, "Train UNet", tooltip="Enables training the UNet model")
        components.switch(frame, 0, 1, self.ui_state, "unet.train")

        # train unet epochs
        components.label(frame, 1, 0, "Stop Training After", tooltip="When to stop training the UNet")
        components.time_entry(
            frame,
            1,
            1,
            ...
            "unet.stop_training_after_unit",
            supports_time_units=False,
        )

        # unet learning rate
        components.label(
            frame, 2, 0, "UNet Learning Rate", tooltip="The learning rate of the UNet. Overrides the base learning rate"
        )
        components.entry(frame, 2, 1, self.ui_state, "unet.learning_rate")

        # rescale noise scheduler to zero terminal SNR
        rescale_label = components.label(
            frame,
            3,
            0,
    ...
    def __create_prior_frame(self, master, row):
        ...
        # train prior
        components.label(frame, 0, 0, "Train Prior", tooltip="Enables training the Prior model")
        components.switch(frame, 0, 1, self.ui_state, "prior.train")

        # train prior epochs
        components.label(frame, 1, 0, "Stop Training After", tooltip="When to stop training the Prior")
        components.time_entry(
            frame,
            1,
            1,
            ...
            "prior.stop_training_after_unit",
            supports_time_units=False,
        )
    ...
    def __create_transformer_frame(
        self, master, row, supports_guidance_scale: bool = False, supports_force_attention_mask: bool = True
    ):
        frame = ctk.CTkFrame(master=master, corner_radius=5)
        frame.grid(row=row, column=0, padx=5, pady=5, sticky="nsew")
        frame.grid_columnconfigure(0, weight=1)
        ...
        # train transformer
        components.label(frame, 0, 0, "Train Transformer", tooltip="Enables training the Transformer model")
        components.switch(frame, 0, 1, self.ui_state, "transformer.train")

        # train transformer epochs
        components.label(frame, 1, 0, "Stop Training After", tooltip="When to stop training the Transformer")
        components.time_entry(
            frame,
            1,
            1,
            ...
            "transformer.stop_training_after_unit",
            supports_time_units=False,
        )
    ...
    def __create_noise_frame(
        ...
        components.label(
            ...
            0,
            "I/O Noise Weight",
            tooltip="Applies Coordinated Input-Output Perturbation (CIOP) to each training step. Recommended 0.01-0.1.",
        )
        ...
        components.label(
            ...
            row,
            0,
            "I/O Noise Probability",
            tooltip="The probability of applying I/O perturbation noise per training step. Recommended 0.5-0.8.",
        )
    ...
    def __create_masked_frame(self, master, row):
        ...
        components.entry(
            ...
            extra_validate=check_range(
                lower=0, upper=1, message="Masked prior preservation weight must be between 0 and 1"
            ),
        )
    ...
    def __create_loss_frame(self, master, row, supports_vb_loss: bool = False):
        ...
        # Huber Delta
        components.label(frame, 4, 0, "Huber Delta", tooltip="Delta parameter for huber loss")
