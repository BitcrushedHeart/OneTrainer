...
class CloudTab:
    def __init__(self, master, train_config: TrainConfig, ui_state: UIState, parent):
        ...
        components.label(self.frame, 0, 0, "Enabled", tooltip="Enable cloud training")
        ...
        components.label(self.frame, 2, 2, "OneTrainer Directory", tooltip="The directory for OneTrainer on the cloud.")
        ...
        components.label(self.frame, 2, 4, "Cloud name", tooltip="The name of the new cloud instance.")
        components.entry(self.frame, 2, 5, self.ui_state, "cloud.name")
        components.label(
            self.frame, 3, 4, "Type", tooltip="Select the RunPod cloud type. See RunPod's website for details."
        )
        components.options_kv(
            self.frame,
            3,
        ...
        components.label(
            self.frame, 4, 4, "GPU", tooltip="Select the GPU type. Enter an API key before pressing the button."
        )

        _, gpu_components = components.options_adv(
            self.frame, 4, 5, [("")], self.ui_state, "cloud.gpu_type", adv_command=self.__set_gpu_types
        )
        ...
        components.label(
            self.frame, 6, 4, "Min download", tooltip="Set the minimum download speed of the cloud in Mbps."
        )
