...
class MultiTrainer(BaseTrainer):
    ...
    def _train_process(
        ...
        for _ in multi.sequential():
            print(
                f"GPU #{multi.rank()}  device: {device} ({torch.cuda.get_device_name()})  "
    ...
    def train(self):
        ...
        workers = torch.multiprocessing.spawn(
            MultiTrainer._train_process, args=(world_size, config_dict, devices), nprocs=world_size - 1, join=False
        )

        multi.set_global_commands(self.commands)
        MultiTrainer._train_process(
            -1, world_size, config_dict, devices, callbacks=self.callbacks
        )  # main process is rank #0
        workers.join()
