        ...
        try:
            ...
            if self.reattach:
                if not self.cloud.can_reattach():
                    raise ValueError(
                        f"There is no detached trainer with run id {self.config.cloud.run_id} on this cloud"
                    )
            else:
                if self.cloud.can_reattach():
                    raise ValueError(
                        f'a detached trainer with id {self.config.cloud.run_id} is still running. Use "Reattach now" to reattach to this trainer!'
                    )
    ...
    def end(self):
        try:
            if (
                ...
                and not self.config.cloud.tensorboard_tunnel
                and not self.config.tensorboard_always_on
            ):
