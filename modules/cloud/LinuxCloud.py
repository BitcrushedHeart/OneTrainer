    ...
    def _connect(self):
        ...
        try:
            self.connection = fabric.Connection(
                host=secrets.host, port=secrets.port, user=secrets.user, connect_kwargs=connect_kwargs
            )
            self.connection.open()
            self.connection.transport.set_keepalive(30)

            self.callback_connection = fabric.Connection(
                host=secrets.host, port=secrets.port, user=secrets.user, connect_kwargs=connect_kwargs
            )

            self.command_connection = fabric.Connection(
                host=secrets.host, port=secrets.port, user=secrets.user, connect_kwargs=connect_kwargs
            )
