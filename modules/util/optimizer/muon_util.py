    ...
    match model.model_type:
        ...
        case _:
            raise NotImplementedError(
                f"Muon optimizer spectral normalization is not implemented for model type: {model.model_type}"
            )
...
def build_muon_adam_key_fn(
    ...
    if config.optimizer.muon_hidden_layers is not None:
        ...
        match model.model_type:
            ...
            case _:  # Unmatched cases
                raise NotImplementedError(
                    f"Default hidden layer patterns are not defined for model type: {model.model_type}"
                )
    ...
    if adam_params_count == 0:
        ...
        print(
            "Consider disabling 'MuonWithAuxAdam' in your configuration since the auxiliary AdamW optimizer is not being used."
        )
...
def split_parameters_for_muon(
    ...
    for group in parameters:
        ...
        if adam_params:
            ...
            base_adam_lr = (
                optimizer_config.muon_adam_lr if optimizer_config.muon_adam_lr is not None else config.learning_rate
            )
