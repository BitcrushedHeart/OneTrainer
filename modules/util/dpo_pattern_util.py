...
def validate_dpo_patterns(chosen_pattern: str, rejected_pattern: str):
    ...
    if bool(chosen_pattern) != bool(rejected_pattern):
        raise ValueError("DPO patterns must be set together: provide both a chosen and a rejected pattern, or neither.")
    for label, pattern in (("chosen", chosen_pattern), ("rejected", rejected_pattern)):
        if not pattern:
            continue
        if pattern.count("{}") != 1 or "{" in pattern.replace("{}", ""):
            raise ValueError(f"DPO {label} pattern '{pattern}' must contain exactly one '{{}}' placeholder.")
        if os.path.isabs(pattern) or pattern.startswith(("/", "\\")):
            raise ValueError(f"DPO {label} pattern '{pattern}' must be relative to the concept path.")
...
def resolve_rejected(
    ...
    if not candidates:
        ...
        raise FileNotFoundError(f"No rejected image for '{stem}': expected '{expected}' under '{concept_path}'.")
