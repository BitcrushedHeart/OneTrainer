    ...
    match config.optimizer.optimizer:
        ...
        case Optimizer.SINKSGD_ADV:
            ...
            optimizer = SinkSGD_adv(
                params=parameters,
                lr=config.learning_rate,
                normed_momentum=optimizer_config.normed_momentum
                if optimizer_config.normed_momentum is not None
                else False,
                momentum=optimizer_config.momentum if optimizer_config.momentum is not None else 0,
                weight_decay=optimizer_config.weight_decay if optimizer_config.weight_decay is not None else 0.0,
                ...
                orthogonal_sinkhorn=optimizer_config.orthogonal_sinkhorn
                if optimizer_config.orthogonal_sinkhorn is not None
                else False,
                sinkhorn_iterations=optimizer_config.sinkhorn_iterations
                if optimizer_config.sinkhorn_iterations is not None
                else 5,
                spectral_normalization=optimizer_config.spectral_normalization
                if optimizer_config.spectral_normalization is not None
                else False,
                centered_wd=optimizer_config.centered_wd if optimizer_config.centered_wd is not None else 0.0,
                centered_wd_mode=optimizer_config.centered_wd_mode
                if optimizer_config.centered_wd_mode is not None
                else "full",
                state_precision=optimizer_config.state_precision
                if optimizer_config.state_precision is not None
                else "auto",
        ...
        case Optimizer.ADOPT_ADV:
            ...
            # adv_optm 2.5.2's Adopt_adv constructor eagerly initializes v0 = grad^2,
            # which crashes when the optimizer is created before the first backward
            # pass (all grads are still None). Give params temporary zero grads, then
            # drop the premature state below so the first real step re-initializes
            # v0 from the true first gradient as ADOPT intends.
            gradless = [p for pg in parameters for p in pg["params"] if p.grad is None]
            for p in gradless:
        ...
        case Optimizer.PRODIGY_ADV:
            ...
            optimizer = Prodigy_adv(
                ...
                k_warmup_steps=(config.learning_rate_warmup_steps / config.gradient_accumulation_steps),
                compiled_optimizer=optimizer_config.compile if optimizer_config.compile is not None else False,
                # Prodigy_adv in adv_optm 2.5.2 does not accept spectral_normalization
                centered_wd=optimizer_config.centered_wd if optimizer_config.centered_wd is not None else 0.0,
                centered_wd_mode=optimizer_config.centered_wd_mode
                if optimizer_config.centered_wd_mode is not None
                else "full",
        ...
        case Optimizer.SIGNSGD_ADV:
            ...
            optimizer = SignSGD_adv(
                params=parameters,
                lr=config.learning_rate,
                stochastic_sign=optimizer_config.stochastic_sign
                if optimizer_config.stochastic_sign is not None
                else False,
                normed_momentum=optimizer_config.normed_momentum
                if optimizer_config.normed_momentum is not None
                else False,
